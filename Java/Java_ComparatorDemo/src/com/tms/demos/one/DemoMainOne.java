...
//		System.out.println("After sorting : "+studentList );
		...
		System.out.println("------------ -------------- ---------");
		System.out.println("After descending sorting BY NAME: " + studentList);

		studentList.sort(Comparator.comparing(Student::getAge).reversed());
		System.out.println("------------ -------------- ---------");
		System.out.println("After descending sorting BY AGE : " + studentList);

		List<Student> listOfFirstClass = studentList.stream()
				.filter(student -> Integer.parseInt(student.getGrade()) >= 60).toList();
